...
const OtpEntry: React.FC<OtpEntryProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-6 bg-slate-50">
      <label className="block mb-2 text-sm font-medium text-slate-700">{contentLabels?.chatbot?.otp?.label || 'Enter OTP'}</label>
      <input
        type="text"
        value={otp}
        ...
        placeholder={contentLabels?.chatbot?.otp?.placeholder || '6-digit OTP'}
        disabled={loading}
      />
      <button
        onClick={onVerifyOtp}
      ...
      >
        {loading ? contentLabels?.chatbot?.otp?.verifying || 'Verifying...' : contentLabels?.chatbot?.otp?.verifyButton || 'Verify OTP'}
      </button>
      <button
        onClick={onChangeEmail}
        className="mt-2 text-xs text-blue-600 hover:underline"
        disabled={loading}
      >{contentLabels?.chatbot?.otp?.changeEmail || 'Change email'}</button>
